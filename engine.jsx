...
/* ---------- reveal manager (rect-based; robust across preview/screenshot envs) ---------- */
const __revealWatchers = new Set();
let __revealRaf = 0;
function __revealTick() {
  const vh = window.innerHeight || document.documentElement.clientHeight;
  __revealWatchers.forEach((w) => {
    const el = w.el;
    if (!el) return;
    const r = el.getBoundingClientRect();
    const visible = r.top < vh * 0.9 && r.bottom > vh * 0.06;
    if (visible && !w.state) {w.state = true;w.set(true);if (w.once) __revealWatchers.delete(w);} else
    if (!visible && w.state && !w.once) {w.state = false;w.set(false);}
  });
}
...
if (typeof window !== "undefined" && !window.__revealBound) {
  ...
  window.addEventListener("scroll", __revealTick, { passive: true });
  window.addEventListener("resize", __revealTick, { passive: true });
}

/* If the document timeline / rAF isn't advancing (offscreen or backgrounded render),
   CSS transitions freeze at their start state and content would stay hidden.
   Detect that and fall back to showing everything statically. */
(function () {
  if (typeof window === "undefined") return;
  let alive = false;
  requestAnimationFrame(() => {alive = true;});
  setTimeout(() => {
    if (!alive) document.documentElement.classList.add("no-anim");
  }, 500);
})();
...
function useInView(opts = {}) {
  ...
  useEffect(() => {
    const w = { el: ref.current, set: setInView, state: false, once: opts.once !== false };
    __revealWatchers.add(w);
    // initial checks (cover late layout / font load / backgrounded rAF throttling)
    __revealTick();
    const t1 = setTimeout(__revealTick, 80);
    const t2 = setTimeout(__revealTick, 450);
    return () => {__revealWatchers.delete(w);clearTimeout(t1);clearTimeout(t2);};
  }, []);
  return [ref, inView];
}

/* ---------- Reveal ----------
   variants: up | right | left | clip | scale | fade  */
function Reveal({ children, variant = "up", delay = 0, as = "div", className = "", style = {}, threshold }) {
...
function BrushDivider({ vertical = false, length = 120, className = "", style = {} }) {
  if (vertical) {
    return (
      ...
      preserveAspectRatio="none" style={style} aria-hidden="true">
        <path d={`M5 2 C 7 ${length * 0.3}, 3 ${length * 0.6}, 5 ${length - 2}`}
        stroke="var(--ink)" strokeWidth="2.4" fill="none" strokeLinecap="round" opacity="0.55" />
      </svg>);
  ...
  return (
    ...
    preserveAspectRatio="none" style={style} aria-hidden="true">
      <path d={`M2 6 C ${length * 0.3} 3, ${length * 0.6} 9, ${length - 2} 5`}
      stroke="var(--ink)" strokeWidth="2.4" fill="none" strokeLinecap="round" opacity="0.55" />
    </svg>);
